<?php
	...
	**************************************************************          
          Modified & expanded for use with Text & Link Destiller V0.5
	   by mckoch@mckoch.de 2009
          
	 additional functions:
          * this->get_word_list();
          * this->get_unique_word_count();
          * modified: class will produce object with all values pre-computed.
	************************************************************** 
	 ...
	 $encoding= 'UTF-8';
        $statistics = new TextStatistics($encoding, $text);
    ...
    */
    ...
    class TextStatistics {
         ...
         * @return void
         */
        public function __construct($strEncoding = '', $strText) {
        	//mising getter for $strText !!!!!!!
            if ($strEncoding <> '') {
                // Encoding is given. Use it!
                $this->strEncoding = $strEncoding;
            }
            $this->strText = $this->clean_text($strText);
            $this->flesch_kincaid_reading_ease=$this->flesch_kincaid_reading_ease($this->strText);
            $this->flesch_kincaid_grade_level=$this->flesch_kincaid_grade_level($this->strText);
            $this->gunning_fog_score=$this->gunning_fog_score($this->strText);
            $this->coleman_liau_index=$this->coleman_liau_index($this->strText);
            $this->smog_index=$this->smog_index($this->strText);
            $this->automated_readability_index=$this->automated_readability_index($this->strText);
            $this->text_length=$this->text_length($this->strText);
            $this->letter_count=$this->letter_count($this->strText);
            $this->sentence_count=$this->sentence_count($this->strText);
            $this->word_count=$this->word_count($this->strText);
            $this->get_unique_word_count=$this->get_unique_word_count($this->strText);
            $this->get_word_list=$this->get_word_list($this->strText);
            $this->get_unique_word_count=$this->get_unique_word_count($this->strText);
            $this->average_words_per_sentence=$this->average_words_per_sentence($this->strText);
            $this->average_syllables_per_word=$this->average_syllables_per_word($this->strText);
            $this->words_with_three_syllables=$this->words_with_three_syllables($this->strText);
            $this->percentage_words_with_three_syllables=$this->percentage_words_with_three_syllables($this->strText);
        }
         ...
         */
        private function flesch_kincaid_reading_ease($strText) {
         ...
         * @param   strText         Text to be checked
         */
        private function flesch_kincaid_grade_level($strText) {
            //$strText = $this->clean_text($strText);
         ...
         * @param   strText         Text to be checked
         */
        private function gunning_fog_score($strText) {
            //$strText = $this->clean_text($strText);
         ...
         * @param   strText         Text to be checked
         */
        private function coleman_liau_index($strText) {
            //$strText = $this->clean_text($strText);
         ...
         * @param   strText         Text to be checked
         */
        private function smog_index($strText) {
            //$strText = $this->clean_text($strText);
         ...
         * @param   strText         Text to be checked
         */
        private function automated_readability_index($strText) {
            //$strText = $this->clean_text($strText);
         ...
         */
        private function text_length($strText) {
            $intTextLength = 0;
            try {
                if ($this->strEncoding == '') {
         ...
         * @param   strText      Text to be measured
         */
        private function letter_count($strText) {
         ...
         * @param   strText      Text to be measured
         */
        private function sentence_count($strText) {
            //$strText = $this->clean_text($strText);
         ...
         * @param   strText      Text to be measured
         */
        private function word_count($strText) {
            //$strText = $this->clean_text($strText);
	...
	private function get_word_list($strText){
		 $string = eregi_replace(" +", " ", $strText);  
	...
	private function get_unique_word_count($strText){
		 $string = eregi_replace(" +", " ", $strText);  
		 $array = explode(" ", $string);  
		 for($i=0;$i < count($array);$i++)  
		{  
         ...
         * @param   strText      Text to be measured
         */
        private function average_words_per_sentence($strText) {
            //$strText = $this->clean_text($strText);
         ...
         * @param   strText      Text to be measured
         */
        private function average_syllables_per_word($strText) {
            //$strText = $this->clean_text($strText);
            $intSyllableCount = 0;
            $intWordCount = $this->word_count($strText);
         ...
         */
        private function words_with_three_syllables($strText, $blnCountProperNouns = true) {
            //$strText = $this->clean_text($strText);
            $intLongWordCount = 0;
            $intWordCount = $this->word_count($strText);
            $arrWords = explode(' ', $strText);
         ...
         * @param   blnCountProperNouns      Boolean - should proper nouns be included in words count
         */
        private function percentage_words_with_three_syllables($strText, $blnCountProperNouns = true) {
            $strText = $this->clean_text($strText);
            $intWordCount = $this->word_count($strText);
         ...
         */
        private function syllable_count($strWord) {
